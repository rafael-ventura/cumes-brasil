...
export class FaceRepository {
    ...
    async getFaces(): Promise<Face[] | null> {
        return new Promise((resolve, reject) => {
            this.db.then((db) => {
                db.all(`SELECT * FROM Face`, (err, rows: Face[]) => {
                    if (err) {
                        reject(err);
                        return;
                    }
    ...
    async createFace(face: Face): Promise<void> {
        return new Promise((resolve, reject) => {
                this.db.then((db) => {
                        db.run(`INSERT INTO Face (nome, montanhaId, fonteId) VALUES (?,?,?)`,
    ...
    async updateFace(face: Face): Promise<void> {
        return new Promise((resolve, reject) => {
                this.db.then((db) => {
                        db.run(`UPDATE Face SET nome = ?, montanhaId = ?, fonteId = ? WHERE id = ?`,
    ...
    async deleteFace(id: number): Promise<void> {
        return new Promise((resolve, reject) => {
            this.db.then((db) => {
                db.run(`DELETE FROM Face WHERE id = ?`, [id], (err) => {
                    if (err) {
                        reject(err);
                        return;
                    }
