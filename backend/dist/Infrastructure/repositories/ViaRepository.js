...
class ViaRepository {
    ...
    async search(query) {
        const { unifiedSearch, selectedMountain, selectedDifficulty, selectedCrux, selectedExtensionCategory, selectedExposicao, colecaoId, bairro, page = 1, itemsPerPage = 10, sortField, sortOrder } = query;
        ...
        if (unifiedSearch) {
            qb = qb.andWhere('(via.nome LIKE :unifiedSearch OR montanha.nome LIKE :unifiedSearch OR montanha.bairro LIKE :unifiedSearch)', { unifiedSearch: `%${unifiedSearch}%` });
        }
