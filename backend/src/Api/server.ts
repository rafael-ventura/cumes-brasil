...
import { DIContainer } from "../Infrastructure/config/container";
...
import {
  ...
  notFoundMiddleware,
} from "./Middlewares/ErrorRequestMiddleware";
import {
  generalRateLimiter,
  getRateLimitInfo,
...
async function ensureDatabaseExists(): Promise<void> {
  ...
  if (!dbName) {
    throw new Error(
      "POSTGRES_DB não está configurado nas variáveis de ambiente",
    );
  }
 ...
 * Tem que ser após configuração da configuração do container
 */
async function setupRoutes() {
  ...
  safeLogger.info("Rotas configuradas com sucesso");
}
...
async function initializeDatabase() {
  try {
    ...
    if (count === 0) {
      safeLogger.info(
        "Nenhum registro encontrado na tabela Via, iniciando carga de dados...",
      );
      await loadData();
      safeLogger.info("Carga inicial realizada com sucesso");
    } else {
      safeLogger.info(
        "Registros já existentes na tabela Via, pulando a carga de dados",
        { count },
      );
...
async function fixSequencesOnStartup() {
  ...
  try {
    safeLogger.info(
      "🔧 Verificando e corrigindo sequências do banco de dados...",
    );
      ...
      try {
        ...
        if (maxId) {
          ...
          if (currentSeq < maxId) {
            ...
            safeLogger.info(
              `  ✅ Sequência ${table}_id_seq ajustada de ${currentSeq} para ${maxId}`,
            );
          }
        }
      } catch (error) {
